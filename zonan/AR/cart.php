...
<section class="whish-list-section theme1 pb-70">
    <div class="container grid-wraper">
        <div class="row">
            <div class="col-12">
                <h3 class="title pb-25 text-center text-md-left text-capitalize" style="float:right">محتويات السلة</h3><br><br>
                    ...
                    <table class="table mb-0">
                        <thead class="thead-light">
                            <tr>
                              ...
                              <th class="text-center" scope="col">المتجر</th>
                              <th class="text-center" scope="col">التوفر</th>
                              <th class="text-center" scope="col">الكمية</th>
                              <th class="text-center" scope="col">السعر</th>
                              <th class="text-center" scope="col">النشاط</th>
                              <th class="text-center" scope="col">اتمام الطلب</th>
                        ...
                        <?php 
                            ...
                            while($row = mysqli_fetch_assoc($data2)){
                             if($row['type'] == 'order'){
                              $_SESSION['orderID'] = $row['uid'];
                                  echo "
                                     <tr class='text-center'>
                                     <th class='text-center' scope='row'>
                                    <img src='assets/img/".$row['proImg']."' alt='img'>
                                </th>
                                <td class='text-center'>
                                <span class=whish-title'>
                            ".$row['proName']."</span></td>
                            <td class='text-center'>
                            <span class='whish-title'>".$row['storeName'] . "<br><img width='50px' height='50px' src='assets/img/" . $row['storeLogo'] ."' alt='img'></span></td>
                                       
                            <td class='text-center'>
                            <span class='badge badge-danger position-static my-badge'>".$row['stock']."</sapn></td>
                            <td class='text-center'>"?>
                            <form method="post" action="checkout.php?id=<?= $row['uid'] ?>">
                                    <div class='product-count style'>
                                        <div class='count d-flex justify-content-center'>
                                         
                                                                                     
                                        <input id='qtn' name="qtn" type='number' min='1' max='10' value='<?php echo $row['qty'] ?>'>
                                      ...
                                      <td class='text-center'>
                                <span class='whish-list-price'>".number_format($row['price'] * $row['qty'],2)."</span></td>
                                 <td class='text-center'>  <a href='cart.php?action=remove&id=".$row['uid']."'>
                           ...
                           <input type="submit" name="buy" value="شراء الان" class='btn theme-btn--dark1 btn--xl text-uppercase'></td></form> 
                                     </tr>
                                   <?php  
...
<section class="whish-list-section theme1 pb-70">
  <div class="container grid-wraper">
      <div class="row">
          <div class="col-12">
              <h3 class="title pb-25 text-center text-md-left text-capitalize" style="float:right">طلباتك</h3><br><br>
              <div class="table-responsive">
                  <table class="table mb-0">
                      <thead class="thead-light">
                          <tr>
                              <th class="text-center" scope="col">المتجر</th>
                              <th class="text-center" scope="col">المنتج</th>
                              <th class="text-center" scope="col">العدد</th>
                              <th class="text-center" scope="col">السعر</th>
                              <th class="text-center" scope="col">رمز الطلب</th>
                          </tr>
                      </thead>
                      <tbody>
                      <?php 
                            $data2 = mysqli_query($conn,"SELECT * FROM $str WHERE email='$_SESSION[email]'");
                            while($row = mysqli_fetch_assoc($data2)){
                             if($row['type'] == 'order2'){
                                          echo" <tr>
                                          <th class='text-center' scope='row'>
                                            <span class='whish-title'>$row[storeName] - <img src='assets/img/$row[storeLogo]' width='50px' alt='img'></span>
                                        </th>
                                          <td class='text-center'>
                                              <span class='whish-title'>$row[proName]</span>
                                          </td>
...
<footer class="bg-lighten2 theme1 position-relative">
  ...
  <!-- search-box and overlay end -->
     ...
     ******************************************************-->
     <script>
      ...
      <script>
        function inc(){
        var qty = document.getElementById('qtn').stepUp();
        
         
        }

        function dec(){
          document.getElementById('qtn').stepDown();
        }
  ...
  $str = "User" . $id;
        ...
        $del = mysqli_query($conn,"DELETE FROM $str WHERE uid='$_SESSION[orderID]'");
